 ...
 * @constructor
 */
const AuthPolicy = (principal, awsAccountId, apiOptions) => {
    /**
...
AuthPolicy.prototype = (function() {
    ...
    return {
        ...
        build: function() {
            ...
            doc['Statement'] = doc['Statemen'].concat(
...
const ValidateToken = (pems, event, context, callback) => {
    ...
    verify(token, pem, { issuer: iss }, function(err, payload) {
        if (err) {
            ...
            const principalId = payload['username'];
