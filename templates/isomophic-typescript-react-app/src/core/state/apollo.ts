...
const errorLink: ApolloLink = onError(({ graphQLErrors, networkError, operation }) => {
  ...
  if (networkError) console.error(`[Network error]: Operation: ${operation}, ${networkError}`);
});
...
export const initializeApolloClient = (fetch: any, uri = '', ssr = false, usePersistedQueries = false): ApolloClient<NormalizedCacheObject> => {
